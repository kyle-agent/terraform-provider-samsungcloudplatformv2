/*
SCP LoadBalancer Service

SCP LoadBalancer Service

API version: 0.0.1.dev1712
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package loadbalancer

import (
	"encoding/json"
	"time"
	"bytes"
	"fmt"
)

// checks if the Loadbalancer type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &Loadbalancer{}

// Loadbalancer struct for Loadbalancer
type Loadbalancer struct {
	// The account ID of the load balancer.
	AccountId string `json:"account_id"`
	// Created At
	CreatedAt time.Time `json:"created_at"`
	// Created By
	CreatedBy string `json:"created_by"`
	Description NullableString `json:"description,omitempty"`
	FirewallId NullableString `json:"firewall_id,omitempty"`
	// Health check IP
	HealthCheckIp []string `json:"health_check_ip"`
	// ID
	Id string `json:"id"`
	// Layer type
	LayerType string `json:"layer_type"`
	// Modified At
	ModifiedAt time.Time `json:"modified_at"`
	// Modified By
	ModifiedBy string `json:"modified_by"`
	// name
	Name string `json:"name"`
	PublicNatId NullableString `json:"public_nat_id,omitempty"`
	// Service IP
	ServiceIp string `json:"service_ip"`
	// Source NAT IP
	SourceNatIp string `json:"source_nat_ip"`
	// The state of the load balancer.
	State string `json:"state"`
	// Subnet ID
	SubnetId string `json:"subnet_id"`
	// VPC ID
	VpcId string `json:"vpc_id"`
}

type _Loadbalancer Loadbalancer

// NewLoadbalancer instantiates a new Loadbalancer object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewLoadbalancer(accountId string, createdAt time.Time, createdBy string, healthCheckIp []string, id string, layerType string, modifiedAt time.Time, modifiedBy string, name string, serviceIp string, sourceNatIp string, state string, subnetId string, vpcId string) *Loadbalancer {
	this := Loadbalancer{}
	this.AccountId = accountId
	this.CreatedAt = createdAt
	this.CreatedBy = createdBy
	this.HealthCheckIp = healthCheckIp
	this.Id = id
	this.LayerType = layerType
	this.ModifiedAt = modifiedAt
	this.ModifiedBy = modifiedBy
	this.Name = name
	this.ServiceIp = serviceIp
	this.SourceNatIp = sourceNatIp
	this.State = state
	this.SubnetId = subnetId
	this.VpcId = vpcId
	return &this
}

// NewLoadbalancerWithDefaults instantiates a new Loadbalancer object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewLoadbalancerWithDefaults() *Loadbalancer {
	this := Loadbalancer{}
	return &this
}

// GetAccountId returns the AccountId field value
func (o *Loadbalancer) GetAccountId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AccountId
}

// GetAccountIdOk returns a tuple with the AccountId field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetAccountIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AccountId, true
}

// SetAccountId sets field value
func (o *Loadbalancer) SetAccountId(v string) {
	o.AccountId = v
}

// GetCreatedAt returns the CreatedAt field value
func (o *Loadbalancer) GetCreatedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.CreatedAt
}

// GetCreatedAtOk returns a tuple with the CreatedAt field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetCreatedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedAt, true
}

// SetCreatedAt sets field value
func (o *Loadbalancer) SetCreatedAt(v time.Time) {
	o.CreatedAt = v
}

// GetCreatedBy returns the CreatedBy field value
func (o *Loadbalancer) GetCreatedBy() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.CreatedBy
}

// GetCreatedByOk returns a tuple with the CreatedBy field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetCreatedByOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.CreatedBy, true
}

// SetCreatedBy sets field value
func (o *Loadbalancer) SetCreatedBy(v string) {
	o.CreatedBy = v
}

// GetDescription returns the Description field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Loadbalancer) GetDescription() string {
	if o == nil || IsNil(o.Description.Get()) {
		var ret string
		return ret
	}
	return *o.Description.Get()
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Loadbalancer) GetDescriptionOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.Description.Get(), o.Description.IsSet()
}

// HasDescription returns a boolean if a field has been set.
func (o *Loadbalancer) HasDescription() bool {
	if o != nil && o.Description.IsSet() {
		return true
	}

	return false
}

// SetDescription gets a reference to the given NullableString and assigns it to the Description field.
func (o *Loadbalancer) SetDescription(v string) {
	o.Description.Set(&v)
}
// SetDescriptionNil sets the value for Description to be an explicit nil
func (o *Loadbalancer) SetDescriptionNil() {
	o.Description.Set(nil)
}

// UnsetDescription ensures that no value is present for Description, not even an explicit nil
func (o *Loadbalancer) UnsetDescription() {
	o.Description.Unset()
}

// GetFirewallId returns the FirewallId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Loadbalancer) GetFirewallId() string {
	if o == nil || IsNil(o.FirewallId.Get()) {
		var ret string
		return ret
	}
	return *o.FirewallId.Get()
}

// GetFirewallIdOk returns a tuple with the FirewallId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Loadbalancer) GetFirewallIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.FirewallId.Get(), o.FirewallId.IsSet()
}

// HasFirewallId returns a boolean if a field has been set.
func (o *Loadbalancer) HasFirewallId() bool {
	if o != nil && o.FirewallId.IsSet() {
		return true
	}

	return false
}

// SetFirewallId gets a reference to the given NullableString and assigns it to the FirewallId field.
func (o *Loadbalancer) SetFirewallId(v string) {
	o.FirewallId.Set(&v)
}
// SetFirewallIdNil sets the value for FirewallId to be an explicit nil
func (o *Loadbalancer) SetFirewallIdNil() {
	o.FirewallId.Set(nil)
}

// UnsetFirewallId ensures that no value is present for FirewallId, not even an explicit nil
func (o *Loadbalancer) UnsetFirewallId() {
	o.FirewallId.Unset()
}

// GetHealthCheckIp returns the HealthCheckIp field value
func (o *Loadbalancer) GetHealthCheckIp() []string {
	if o == nil {
		var ret []string
		return ret
	}

	return o.HealthCheckIp
}

// GetHealthCheckIpOk returns a tuple with the HealthCheckIp field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetHealthCheckIpOk() ([]string, bool) {
	if o == nil {
		return nil, false
	}
	return o.HealthCheckIp, true
}

// SetHealthCheckIp sets field value
func (o *Loadbalancer) SetHealthCheckIp(v []string) {
	o.HealthCheckIp = v
}

// GetId returns the Id field value
func (o *Loadbalancer) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *Loadbalancer) SetId(v string) {
	o.Id = v
}

// GetLayerType returns the LayerType field value
func (o *Loadbalancer) GetLayerType() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.LayerType
}

// GetLayerTypeOk returns a tuple with the LayerType field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetLayerTypeOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.LayerType, true
}

// SetLayerType sets field value
func (o *Loadbalancer) SetLayerType(v string) {
	o.LayerType = v
}

// GetModifiedAt returns the ModifiedAt field value
func (o *Loadbalancer) GetModifiedAt() time.Time {
	if o == nil {
		var ret time.Time
		return ret
	}

	return o.ModifiedAt
}

// GetModifiedAtOk returns a tuple with the ModifiedAt field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetModifiedAtOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ModifiedAt, true
}

// SetModifiedAt sets field value
func (o *Loadbalancer) SetModifiedAt(v time.Time) {
	o.ModifiedAt = v
}

// GetModifiedBy returns the ModifiedBy field value
func (o *Loadbalancer) GetModifiedBy() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ModifiedBy
}

// GetModifiedByOk returns a tuple with the ModifiedBy field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetModifiedByOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ModifiedBy, true
}

// SetModifiedBy sets field value
func (o *Loadbalancer) SetModifiedBy(v string) {
	o.ModifiedBy = v
}

// GetName returns the Name field value
func (o *Loadbalancer) GetName() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Name
}

// GetNameOk returns a tuple with the Name field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetNameOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Name, true
}

// SetName sets field value
func (o *Loadbalancer) SetName(v string) {
	o.Name = v
}

// GetPublicNatId returns the PublicNatId field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *Loadbalancer) GetPublicNatId() string {
	if o == nil || IsNil(o.PublicNatId.Get()) {
		var ret string
		return ret
	}
	return *o.PublicNatId.Get()
}

// GetPublicNatIdOk returns a tuple with the PublicNatId field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *Loadbalancer) GetPublicNatIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return o.PublicNatId.Get(), o.PublicNatId.IsSet()
}

// HasPublicNatId returns a boolean if a field has been set.
func (o *Loadbalancer) HasPublicNatId() bool {
	if o != nil && o.PublicNatId.IsSet() {
		return true
	}

	return false
}

// SetPublicNatId gets a reference to the given NullableString and assigns it to the PublicNatId field.
func (o *Loadbalancer) SetPublicNatId(v string) {
	o.PublicNatId.Set(&v)
}
// SetPublicNatIdNil sets the value for PublicNatId to be an explicit nil
func (o *Loadbalancer) SetPublicNatIdNil() {
	o.PublicNatId.Set(nil)
}

// UnsetPublicNatId ensures that no value is present for PublicNatId, not even an explicit nil
func (o *Loadbalancer) UnsetPublicNatId() {
	o.PublicNatId.Unset()
}

// GetServiceIp returns the ServiceIp field value
func (o *Loadbalancer) GetServiceIp() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.ServiceIp
}

// GetServiceIpOk returns a tuple with the ServiceIp field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetServiceIpOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.ServiceIp, true
}

// SetServiceIp sets field value
func (o *Loadbalancer) SetServiceIp(v string) {
	o.ServiceIp = v
}

// GetSourceNatIp returns the SourceNatIp field value
func (o *Loadbalancer) GetSourceNatIp() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SourceNatIp
}

// GetSourceNatIpOk returns a tuple with the SourceNatIp field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetSourceNatIpOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SourceNatIp, true
}

// SetSourceNatIp sets field value
func (o *Loadbalancer) SetSourceNatIp(v string) {
	o.SourceNatIp = v
}

// GetState returns the State field value
func (o *Loadbalancer) GetState() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.State
}

// GetStateOk returns a tuple with the State field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetStateOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.State, true
}

// SetState sets field value
func (o *Loadbalancer) SetState(v string) {
	o.State = v
}

// GetSubnetId returns the SubnetId field value
func (o *Loadbalancer) GetSubnetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.SubnetId
}

// GetSubnetIdOk returns a tuple with the SubnetId field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetSubnetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.SubnetId, true
}

// SetSubnetId sets field value
func (o *Loadbalancer) SetSubnetId(v string) {
	o.SubnetId = v
}

// GetVpcId returns the VpcId field value
func (o *Loadbalancer) GetVpcId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.VpcId
}

// GetVpcIdOk returns a tuple with the VpcId field value
// and a boolean to check if the value has been set.
func (o *Loadbalancer) GetVpcIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.VpcId, true
}

// SetVpcId sets field value
func (o *Loadbalancer) SetVpcId(v string) {
	o.VpcId = v
}

func (o Loadbalancer) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o Loadbalancer) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["account_id"] = o.AccountId
	toSerialize["created_at"] = o.CreatedAt
	toSerialize["created_by"] = o.CreatedBy
	if o.Description.IsSet() {
		toSerialize["description"] = o.Description.Get()
	}
	if o.FirewallId.IsSet() {
		toSerialize["firewall_id"] = o.FirewallId.Get()
	}
	toSerialize["health_check_ip"] = o.HealthCheckIp
	toSerialize["id"] = o.Id
	toSerialize["layer_type"] = o.LayerType
	toSerialize["modified_at"] = o.ModifiedAt
	toSerialize["modified_by"] = o.ModifiedBy
	toSerialize["name"] = o.Name
	if o.PublicNatId.IsSet() {
		toSerialize["public_nat_id"] = o.PublicNatId.Get()
	}
	toSerialize["service_ip"] = o.ServiceIp
	toSerialize["source_nat_ip"] = o.SourceNatIp
	toSerialize["state"] = o.State
	toSerialize["subnet_id"] = o.SubnetId
	toSerialize["vpc_id"] = o.VpcId
	return toSerialize, nil
}

func (o *Loadbalancer) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"account_id",
		"created_at",
		"created_by",
		"health_check_ip",
		"id",
		"layer_type",
		"modified_at",
		"modified_by",
		"name",
		"service_ip",
		"source_nat_ip",
		"state",
		"subnet_id",
		"vpc_id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varLoadbalancer := _Loadbalancer{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varLoadbalancer)

	if err != nil {
		return err
	}

	*o = Loadbalancer(varLoadbalancer)

	return err
}

type NullableLoadbalancer struct {
	value *Loadbalancer
	isSet bool
}

func (v NullableLoadbalancer) Get() *Loadbalancer {
	return v.value
}

func (v *NullableLoadbalancer) Set(val *Loadbalancer) {
	v.value = val
	v.isSet = true
}

func (v NullableLoadbalancer) IsSet() bool {
	return v.isSet
}

func (v *NullableLoadbalancer) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableLoadbalancer(val *Loadbalancer) *NullableLoadbalancer {
	return &NullableLoadbalancer{value: val, isSet: true}
}

func (v NullableLoadbalancer) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableLoadbalancer) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


