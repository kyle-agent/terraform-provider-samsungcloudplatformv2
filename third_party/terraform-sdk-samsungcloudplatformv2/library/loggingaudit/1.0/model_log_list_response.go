/*
OSSCP Logging Audit Service

OSSCP Logging Audit Service

API version: 1.2.2.dev159
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package loggingaudit

import (
	"bytes"
	"encoding/json"
	"fmt"
)

// checks if the LogListResponse type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &LogListResponse{}

// LogListResponse struct for LogListResponse
type LogListResponse struct {
	// count
	Count int32 `json:"count"`
	Logs  []Log `json:"logs"`
	// page
	Page int32 `json:"page"`
	// size
	Size int32    `json:"size"`
	Sort []string `json:"sort,omitempty"`
}

type _LogListResponse LogListResponse

// NewLogListResponse instantiates a new LogListResponse object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewLogListResponse(count int32, logs []Log, page int32, size int32) *LogListResponse {
	this := LogListResponse{}
	this.Count = count
	this.Logs = logs
	this.Page = page
	this.Size = size
	return &this
}

// NewLogListResponseWithDefaults instantiates a new LogListResponse object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewLogListResponseWithDefaults() *LogListResponse {
	this := LogListResponse{}
	return &this
}

// GetCount returns the Count field value
func (o *LogListResponse) GetCount() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Count
}

// GetCountOk returns a tuple with the Count field value
// and a boolean to check if the value has been set.
func (o *LogListResponse) GetCountOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Count, true
}

// SetCount sets field value
func (o *LogListResponse) SetCount(v int32) {
	o.Count = v
}

// GetLogs returns the Logs field value
func (o *LogListResponse) GetLogs() []Log {
	if o == nil {
		var ret []Log
		return ret
	}

	return o.Logs
}

// GetLogsOk returns a tuple with the Logs field value
// and a boolean to check if the value has been set.
func (o *LogListResponse) GetLogsOk() ([]Log, bool) {
	if o == nil {
		return nil, false
	}
	return o.Logs, true
}

// SetLogs sets field value
func (o *LogListResponse) SetLogs(v []Log) {
	o.Logs = v
}

// GetPage returns the Page field value
func (o *LogListResponse) GetPage() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Page
}

// GetPageOk returns a tuple with the Page field value
// and a boolean to check if the value has been set.
func (o *LogListResponse) GetPageOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Page, true
}

// SetPage sets field value
func (o *LogListResponse) SetPage(v int32) {
	o.Page = v
}

// GetSize returns the Size field value
func (o *LogListResponse) GetSize() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Size
}

// GetSizeOk returns a tuple with the Size field value
// and a boolean to check if the value has been set.
func (o *LogListResponse) GetSizeOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Size, true
}

// SetSize sets field value
func (o *LogListResponse) SetSize(v int32) {
	o.Size = v
}

// GetSort returns the Sort field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *LogListResponse) GetSort() []string {
	if o == nil {
		var ret []string
		return ret
	}
	return o.Sort
}

// GetSortOk returns a tuple with the Sort field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *LogListResponse) GetSortOk() ([]string, bool) {
	if o == nil || IsNil(o.Sort) {
		return nil, false
	}
	return o.Sort, true
}

// HasSort returns a boolean if a field has been set.
func (o *LogListResponse) HasSort() bool {
	if o != nil && !IsNil(o.Sort) {
		return true
	}

	return false
}

// SetSort gets a reference to the given []string and assigns it to the Sort field.
func (o *LogListResponse) SetSort(v []string) {
	o.Sort = v
}

func (o LogListResponse) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o LogListResponse) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["count"] = o.Count
	toSerialize["logs"] = o.Logs
	toSerialize["page"] = o.Page
	toSerialize["size"] = o.Size
	if o.Sort != nil {
		toSerialize["sort"] = o.Sort
	}
	return toSerialize, nil
}

func (o *LogListResponse) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"count",
		"logs",
		"page",
		"size",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varLogListResponse := _LogListResponse{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varLogListResponse)

	if err != nil {
		return err
	}

	*o = LogListResponse(varLogListResponse)

	return err
}

type NullableLogListResponse struct {
	value *LogListResponse
	isSet bool
}

func (v NullableLogListResponse) Get() *LogListResponse {
	return v.value
}

func (v *NullableLogListResponse) Set(val *LogListResponse) {
	v.value = val
	v.isSet = true
}

func (v NullableLogListResponse) IsSet() bool {
	return v.isSet
}

func (v *NullableLogListResponse) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableLogListResponse(val *LogListResponse) *NullableLogListResponse {
	return &NullableLogListResponse{value: val, isSet: true}
}

func (v NullableLogListResponse) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableLogListResponse) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
