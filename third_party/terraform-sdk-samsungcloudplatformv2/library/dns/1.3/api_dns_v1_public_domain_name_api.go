...
type DnsV1PublicDomainNameApiAPITransferPublicDomainRequest struct {
	ctx                         context.Context
	ApiService                  *DnsV1PublicDomainNameApiAPIService
	publicDomainId              string
	publicDomainTransferRequest *PublicDomainTransferRequest
}

func (r DnsV1PublicDomainNameApiAPITransferPublicDomainRequest) PublicDomainTransferRequest(publicDomainTransferRequest PublicDomainTransferRequest) DnsV1PublicDomainNameApiAPITransferPublicDomainRequest {
	r.publicDomainTransferRequest = &publicDomainTransferRequest
	return r
}

func (r DnsV1PublicDomainNameApiAPITransferPublicDomainRequest) Execute() (*PublicDomainTransferResponse, *http.Response, error) {
	return r.ApiService.TransferPublicDomainExecute(r)
}

/*
TransferPublicDomain Transfer Public Domain (request/cancel/approve/reject)

Public domain transfer workflow between projects

	@param ctx context.Context - for authentication, logging, cancellation, deadlines, tracing, etc. Passed from http.Request or context.Background().
	@param publicDomainId Id
	@return DnsV1PublicDomainNameApiAPITransferPublicDomainRequest
*/
func (a *DnsV1PublicDomainNameApiAPIService) TransferPublicDomain(ctx context.Context, publicDomainId string) DnsV1PublicDomainNameApiAPITransferPublicDomainRequest {
	return DnsV1PublicDomainNameApiAPITransferPublicDomainRequest{
		ApiService:     a,
		ctx:            ctx,
		publicDomainId: publicDomainId,
...
// Execute executes the request
//
//	@return PublicDomainTransferResponse
func (a *DnsV1PublicDomainNameApiAPIService) TransferPublicDomainExecute(r DnsV1PublicDomainNameApiAPITransferPublicDomainRequest) (*PublicDomainTransferResponse, *http.Response, error) {
	var (
		localVarHTTPMethod  = http.MethodPost
		localVarPostBody    interface{}
		formFiles           []formFile
		localVarReturnValue *PublicDomainTransferResponse
	)
	...
	localVarPath := basePath + "/v1/public-domain-names/{public_domain_id}/transfer"
	localVarPath = strings.Replace(localVarPath, "{"+"public_domain_id"+"}", url.PathEscape(parameterValueToString(r.publicDomainId, "publicDomainId")), -1)

	localVarHeaderParams := make(map[string]string)
	localVarQueryParams := url.Values{}
	localVarFormParams := url.Values{}
	if r.publicDomainTransferRequest == nil {
		return localVarReturnValue, nil, reportError("publicDomainTransferRequest is required and must be specified")
	}
	...
	// body params
	localVarPostBody = r.publicDomainTransferRequest
	if r.ctx != nil {
		// API Key Authentication
		if auth, ok := r.ctx.Value(ContextAPIKeys).(map[string]APIKey); ok {
			if apiKey, ok := auth["X-Auth-Token"]; ok {
