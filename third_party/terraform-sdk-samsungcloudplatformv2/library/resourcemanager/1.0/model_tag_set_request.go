/*
SCP Resource manager project

SCP Resource manager project

API version: 1.3.0.0rc1.dev201
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package resourcemanager

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the TagSetRequest type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &TagSetRequest{}

// TagSetRequest struct for TagSetRequest
type TagSetRequest struct {
	// 태그 목록
	Tags []Tag `json:"tags"`
}

type _TagSetRequest TagSetRequest

// NewTagSetRequest instantiates a new TagSetRequest object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewTagSetRequest(tags []Tag) *TagSetRequest {
	this := TagSetRequest{}
	this.Tags = tags
	return &this
}

// NewTagSetRequestWithDefaults instantiates a new TagSetRequest object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewTagSetRequestWithDefaults() *TagSetRequest {
	this := TagSetRequest{}
	return &this
}

// GetTags returns the Tags field value
func (o *TagSetRequest) GetTags() []Tag {
	if o == nil {
		var ret []Tag
		return ret
	}

	return o.Tags
}

// GetTagsOk returns a tuple with the Tags field value
// and a boolean to check if the value has been set.
func (o *TagSetRequest) GetTagsOk() ([]Tag, bool) {
	if o == nil {
		return nil, false
	}
	return o.Tags, true
}

// SetTags sets field value
func (o *TagSetRequest) SetTags(v []Tag) {
	o.Tags = v
}

func (o TagSetRequest) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o TagSetRequest) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["tags"] = o.Tags
	return toSerialize, nil
}

func (o *TagSetRequest) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"tags",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varTagSetRequest := _TagSetRequest{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varTagSetRequest)

	if err != nil {
		return err
	}

	*o = TagSetRequest(varTagSetRequest)

	return err
}

type NullableTagSetRequest struct {
	value *TagSetRequest
	isSet bool
}

func (v NullableTagSetRequest) Get() *TagSetRequest {
	return v.value
}

func (v *NullableTagSetRequest) Set(val *TagSetRequest) {
	v.value = val
	v.isSet = true
}

func (v NullableTagSetRequest) IsSet() bool {
	return v.isSet
}

func (v *NullableTagSetRequest) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableTagSetRequest(val *TagSetRequest) *NullableTagSetRequest {
	return &NullableTagSetRequest{value: val, isSet: true}
}

func (v NullableTagSetRequest) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableTagSetRequest) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


